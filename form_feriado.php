<?php
	...
	$puede_entrar = $bd->checkAccess($_SESSION["id_usuario"],34,''); //34-Feriados
...
function enviar(inForm){
	if (document.frmABMC.opcion.value != 2){ //SI NO ELIJE ELIMINAR
		...
		if (inForm.fecha.value.trim() == "") 
		{
			alert("Por favor, complete el fecha del feriado.");
			inForm.fecha.focus();
			return false;
		}
		else if (inForm.descripcion.value.trim() == "") 
		{
			alert("Por favor, complete el descripcion del feriado.");
			inForm.descripcion.focus();
			return false;
		}
		else
			inForm.submit();
...
<table width="898" height="346" border="0" align="center" cellpadding="0">
  <tr>
    <td class="cerrar"><p><strong><img src="images/bullet20.gif" width="9" height="9" /> <?php echo 'Usuario: '. $nombre_usuario ?></strong></p>
      <p>&nbsp;</p></td>
    <td align="left" valign="middle"><span class="TITULO">:: Feriados ::</span><a href="form_feriado.php?opcion=1"><img src="agregar.png" width="25" height="25" border="0"></a> 
    </td>
  </tr>
  <tr>
    ...
    <td width="722" valign="top"> <p> 
		...
		switch ($opcion){
			...
			case 4: //CONSULTA
				$id_feriado= $_GET['id'];
				$row = $feriados->consultar_feriado($id_feriado);
				$fecha = $row['fecha'];				
...
?>
      ...
      <form action="abm_feriado.php" method="post" enctype="multipart/form-data" name="frmABMC">
        <table align="center" class="tabla_form">
          ...
          <tr> 
            ...
            <?php 
		echo '<td class="modo1"><input name="fecha" type="date" value="' . $fecha .'"' . $textoReadOnly. '></td>';
	  ?>
          </tr>
          <tr> 
            <td class="modo1"><div align="right"><font color="#000099"><strong>Descripcion*:</strong></font></div></td>
            <?php 
		echo '<td class="modo2"><input name="descripcion" type="text"  value="' . $descripcion .'"' . 'size="55" maxlength="100" '.$textoReadOnly. '></td>';
	  ?>
          </tr>
          <tr> 
        ...
        <?php	
		echo '<input type="hidden" name="id_feriado" value="'.$id_feriado.'">';				
