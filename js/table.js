...
var Sort = (function(){
	...
	sort.date.formats = [
		...
		//Las dos de arriba son las lineas originales, las modifico para que ordenen dia-mes-año en estas dos de abajo
		// DD-MM-YY[YY] HH:MI:SS
		,{ re:/(\d{2})-(\d{2})-(\d{4})\s(\d{2}):(\d{2}):(\d{2})/ , f:function(x){ return (new Date(x[3],x[2]-1,x[1],x[4],x[5],x[6])).getTime(); } } 
		//dd/mm/yyyy
		,{ re:/(\d{2})[\/](\d{2})[\/](\d{4})/ , f:function(x){ return (new Date(x[3],x[2]-1,x[1])).getTime(); } } 
				...
				if (colValues.length>0) {
					if (typeof(args.insert)=="function") {
						...
						if (colValues.length<10) {
							...
							sel += '</select>';
							cell.innerHTML = sel+"<br>"+cell.innerHTML;
						}
						else {
							var inp = '<input size="10" onkeyup="Table.filter(this,this)" class="'+table.AutoFilterClassName+'" onclick="Table.cancelBubble(event)">';
							cell.innerHTML = inp+"<br>"+cell.innerHTML;
							}
					}
				}
			}
...
//o = (document.getElementById("filterGrid"));
//alert(o.attachEvent("click", table.auto));//vanina
//o.addEventListener("click", table.auto, false);//vanina
	return table;
})();

function doPagination(tableId, page) {
...
function sumup(tableId, totalRow) {
     var t = [], cell,
      row = document.getElementById(tableId).rows,
      i = row.length - totalRow, //Ante ultima fila es la de totales
      lastrow = row[i];
     while(i--) {
      cell = row[i].cells; j = cell.length;
      while(j--) {
       if( !t[j] ) { t[j] = 0; }
       t[j] += parseFloat( cell[j].innerHTML ) || 0;
      }
     }
     j = t.length; 
     while(j--) {
      if( lastrow.cells[j] ) { 
	if (lastrow.cells[j].innerHTML == "") { //Solo pone los totales en las columnas que no tienen contenido
		lastrow.cells[j].innerHTML = t[j]; }
	}
     }
    }
