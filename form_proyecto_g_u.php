<?php
	...
	//$puede_entrar = $bd->getPermisos($userData,'CAN_ACCESS_UNIHDO');
	$puede_entrar = $bd->checkAccess($_SESSION["id_usuario"],23,''); //23-web unhido
...
<table width="898" height="346" border="0" align="center" cellpadding="0">
  <tr>
    ...
    <td align="left" valign="middle"><span class="TITULO">:: 
      Proyectos Gesti&oacute;n UNIDHO ::</span>
	  <?php
		$webPGUAlta = $bd->checkPerm($_SESSION["id_usuario"],23,'alta');
		echo '<a href="'; 
		echo ($webPGUAlta)?"form_proyecto_g_u.php?opcion=1":"#"; 
		echo '"><img src="';
		echo ($webPGUAlta)?"agregar.png":"iconos_grises/agregarg.png";
    ...
    <td width="722" valign="top"> <p> 
		...
		switch ($opcion){
			...
			case 3: // OPCION MODIFICACION
				$id_proyecto_g_u = $_GET['id_proyecto_g_u'];
...
?>
      ...
      <form action="abm_proyecto_g_u.php" method="post" enctype="multipart/form-data" name="form3" id="form3">
	  ...
	  ?>
		...
		echo '<br>'.substr($archivo, strlen($id_proyecto_g_u)-1).'</td>';		
		if ( ($opcion ==3) && ($archivo) ){
	...
	?>
		...
		switch ($opcion){
			...
			case 2: // BAJA 
				...
				echo '<input type="hidden" name="id_proyecto_g_u" id="id_proyecto_g_u" value="'.$id_proyecto_g_u.'">';				
