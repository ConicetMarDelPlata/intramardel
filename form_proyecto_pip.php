<?php
	...
	//$puede_entrar = $bd->getPermisos($userData,'CAN_ACCES_PIP');
	$puede_entrar = $bd->checkAccess($_SESSION["id_usuario"],20,''); //20-web pip
...
<table width="898" height="346" border="0" align="center" cellpadding="0">
  <tr>
    ...
    <td align="left" valign="middle"><span class="TITULO">:: 
      Proyectos PIP ::</span>	  
	  <?php
		$webPIPAlta = $bd->checkPerm($_SESSION["id_usuario"],20,'alta');
		echo '<a href="'; 
		echo ($webPIPAlta)?"form_proyecto_pip.php?opcion=1":"#"; 
		echo '"><img src="';
		echo ($webPIPAlta)?"agregar.png":"iconos_grises/agregarg.png";
    ...
    <td width="722" valign="top"> <p> 
		...
		switch ($opcion){
			...
			case 3: // OPCION MODIFICACION
				$id_proyecto_pip = $_GET['id_proyecto_pip'];
...
?>
      ...
      <form action="abm_proyecto_pip.php" method="post" enctype="multipart/form-data" name="form3" id="form3">
	  ...
	  ?>
		...
		echo '<br>'.$archivo.'</td>';
		/* echo '<br>'.substr($archivo, count($archivo)).'</td>'; NO FUNCIONA EN PHP 7 */
		if ( ($opcion ==3) && ($archivo) ){
	...
	?>
		...
		switch ($opcion){
			...
			case 2: // BAJA 
				...
				echo '<input type="hidden" name="id_proyecto_pip" id="id_proyecto_pip" value="'.$id_proyecto_pip.'">';				
